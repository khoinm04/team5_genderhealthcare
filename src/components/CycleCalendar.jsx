  ...
  <div className="mb-6">
    ...
    <div className="grid grid-cols-7 gap-1 mb-4">
      ...
      {getMonthCalendar(cycleCalendarMonth).map((day, index) => {
        const dayType = getCycleDayType(
          cycleCalendarMonth.year,
          cycleCalendarMonth.month,
          day
        );
          ...
          <div
            ...
            className={`aspect-square flex items-center justify-center text-sm font-medium rounded-lg border ${
              !day
                ? ''
                : dayType === 'period'
                ? 'bg-red-400 text-white' // 🔴 Ngày kinh nguyệt
                : dayType === 'ovulation'
                ? 'bg-green-500 text-white' // 🟢 Ngày rụng trứng
                : dayType === 'high-fertility'
                ? 'bg-yellow-300 text-gray-800' // 🟡 Dễ thụ thai cao
                : dayType === 'medium-fertility'
                ? 'bg-orange-300 text-gray-800' // 🟠 Dễ thụ thai trung bình
                : dayType === 'low-fertility'
                ? 'bg-purple-200 text-gray-800' // 🟣 Dễ thụ thai thấp
                : 'bg-gray-100 text-gray-600' // ❔ Ngày thường
            }`}
          >
            {day}
          </div>
    ...
    {hasCycleData && (
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>🟡 Dễ thụ thai cao</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-orange-300 rounded"></div>
          <span>🟠 Dễ thụ thai trung bình</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 bg-purple-200 rounded"></div>
          <span>🟣 Dễ thụ thai thấp</span>
        </div>
      </div>
    )}
  </div>
