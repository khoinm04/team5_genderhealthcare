...
useEffect(() => {
  ...
  if (storedUser) {
    try {
      ...
      setIsGoogleAccount(parsedUser.authProvider === "GOOGLE");
    } catch (e) {
      console.error("Lỗi parse authProvider:", e);
    }
  }
}, []);
  ...
  axios.put('http://localhost:8080/gender-health-care/profile', {
    ...
    .catch(err => {
      alert(err.response?.data || "Lưu thông tin thất bại.");
    });
};
  ...
  axios.put('http://localhost:8080/gender-health-care/change-password', null, {
    params: {
      ...
      <div className="space-y-6">
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          ...
          <p className="text-sm text-yellow-700">
            Bạn đang sử dụng đăng nhập bằng Google. Tính năng đổi mật khẩu không khả dụng với tài khoản Google.
          </p>
        </div>
      </div>
    );
